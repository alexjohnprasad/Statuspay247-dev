...
function updateTaskScreenshotLink(adId, phone, fileUrl) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (rowAdId === adId && rowPhone === standardizedPhone) {
      ...
      const existingCell =
        sheet.getRange(row, 9).getFormula() ||
      ...
      if (uniqueOldUrls.length > 0) {
        ...
        const oldLinksFormulas = uniqueOldUrls
          .map((url, idx) => {
...
function cleanupOldFiles(folder) {
  try {
    Logger.log(
      'cleanupOldFiles: folder id = ' +
        (folder ? folder.getId() : 'null or undefined')
    );
    if (!folder) {
      ...
      if (cell) {
        ...
        lines.forEach((line) => {
          const match = line.match(
            /https?:\/\/drive\.google\.com\/uc\?id=[-\w]{25,}/
          );
...
function doPost(e) {
  try {
    ...
    // Create and validate blob with filename
    const filename = requestData.filename || `upload_${Date.now()}`;
    const rawBlob = Utilities.newBlob(
    ...
    const finalFileName = `completed_task_${adId}_${phone}_${timestamp}.${getFileExtensionFromBlob(
      fileBlob
    )}`;
